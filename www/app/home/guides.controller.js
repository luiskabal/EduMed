(function() {
    ...
    function guidesController($scope,$state,$ionicHistory,commonService,$rootScope,guidesFactory) {
        ...
        vm.textSearch = "";
        $scope.$watch('guidesCtrl.textSearch', function() {

            if(vm.textSearch !== ""){
                console.log(vm.textSearch);
                var listaTemporal = [];
                angular.forEach(vm.backUpGuides, function(current) {
                    if(removeDiacritics(current.titulo).toUpperCase().indexOf(removeDiacritics(vm.textSearch).toUpperCase()) !== -1 || removeDiacritics(current.subtitulo).toUpperCase().indexOf(removeDiacritics(vm.textSearch).toUpperCase()) !== -1) {
                        console.log(current);
                        console.log(current.titulo);
                        console.log(current.subtitulo);
                        console.log(current.descripcion);
                        listaTemporal.push(current);
                    }
                });
                vm.guides = listaTemporal;
            }else {
                console.log("vacio");
                vm.guides = vm.backUpGuides;
            }
        });
